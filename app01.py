...
class User(UserMixin, db.Model):
    ...
    # 修改为多个佩戴称号
    wearing_titles = db.relationship('Title', 
                                     secondary='user_wearing_titles',
                                     lazy='dynamic',
                                     backref=db.backref('wearing_users', lazy='dynamic'),
                                     order_by="user_wearing_titles.c.display_order")
    ...
    def get_wearing_titles(self):
        """获取用户正在佩戴的所有称号"""
        # 直接查询 user_wearing_titles 关联表获取用户佩戴的所有称号
        titles = Title.query.join(user_wearing_titles).filter(
            user_wearing_titles.c.user_id == self.id
        ).order_by(user_wearing_titles.c.display_order).all()
        
        # 如果没有记录，但有兼容字段，则返回旧系统的称号
        if not titles and self.wearing_title_id:
...
class Post(db.Model):
    ...
    # 添加 comments 关系定义，与 Comment 模型相对应
    comments = db.relationship('Comment', back_populates='post', overlaps="comments_list")
...
class Comment(db.Model):
    ...
    # 修改关系定义，使用back_populates而非backref
...
class CommentImage(db.Model):
    ...
    # 关系定义
    comment = db.relationship('Comment', backref=db.backref('images', cascade='all, delete-orphan'))
...
def load_user(user_id):
    ...
    if user:
        ...
        user.wearing_title = db.session.get(Title, user.wearing_title_id)
    return user
    ...
    if current_user.is_authenticated:
        try:
            ...
            if current_user.wearing_title_id and not hasattr(current_user, '_wearing_title_loaded'):
                current_user.wearing_title = db.session.get(Title, current_user.wearing_title_id)
...
def update_wearing_titles():
    ...
    except Exception as e:  # 修复: 将except与try对齐
        db.session.rollback()
        app.logger.error(f"更新称号设置失败: {str(e)}")
...
# 添加 profile_view 路由别名
@app.route('/profile/<username>/view')
def profile_view(username):
    # 简单地重定向到原始 profile 路由
    return redirect(url_for('profile', username=username))


def ensure_comment_fields():
    """确保comment表有所需的字段"""
    with app.app_context():
        try:
            from sqlalchemy import text
            ...
            if 'parent_id' not in columns:
                db.session.execute(text("ALTER TABLE comment ADD COLUMN parent_id INTEGER REFERENCES comment(id)"))
                print("添加了parent_id字段")
            
            db.session.commit()
            print("评论表结构检查完成")
        except Exception as e:
            db.session.rollback()
            print(f"检查评论表结构时出错: {e}")       
# 在应用启动时检查字段
...
if __name__ == '__main__':
    ...
    # 创建必要的子目录
    for folder in ['attachments', 'backgrounds', 'comments']:
